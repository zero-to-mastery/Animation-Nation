...
const IS_MAINTAINER = [ 'admin', 'maintain' ].includes(process.env?.GITHUB_PERMISSION_ROLE || '')
...
const getContributionState = () => {
    ...
    const requiredFilesRegexp  = new RegExp(`(${EXPECTED_HTML}|${EXPECTED_CSS}|${EXPECTED_JSON})$`)
...
const checkHTML = (file, fileContent, feedbackPush) => {
  ...
  if( !file.includes(EXPECTED_HTML) ){
    feedbackPush(`Incorrect filename for \`${file}\`: please **rename** it as recommended`)
  }
    ...
    feedbackPush(`Empty file, add the code for the file \`${file}\``)
      ...
      if (!hasCSS) {
        feedbackPush('Missing linked stylesheet file: link the CSS file to your HTML')
         ...
         feedbackPush('Incorrect stylesheet link: update the `href` so it points to the correct CSS file')
      }
  }
}
...
const checkCSS = (file, fileContent, feedbackPush) => {
    ...
    if( !file.includes(EXPECTED_CSS) ){
        feedbackPush(`Incorrect filename: please rename as recommended or remove if unnecessary`)
    }
    ...
    if( !fileContent ){
        ...
        feedbackPush('Missing animation: include at least one `\@keyframes` definition and use the `animation` property')
    }
}
...
const checkJSON = (file, fileContent, feedbackPush) => {
    ...
    if( !fileContent ){
        ...
        if( !hasMetaArtNameValue ) feedbackPush('Missing `artName`: please include an `artName` value')
    
        if( hasMetaGithubHandleValue && !hasCorrectMetaGithubHandle ){
            feedbackPush('Unmatched `githubHandle`: make sure it matches your GitHub username')
        ...
        if( hasMoreProperties ){
            feedbackPush('Remove extra properties — only `githubHandle` and `artName` should be present')
        }
    }
...
const reviewOverallContribution = (contributionStates) => {
    ...
    const artFolderFiles = changedFiles.filter(filename => filename.startsWith('Art/'))
    ...
    const isMaintainerAsContributor = artFolderFiles.some(f => f.startsWith('Art/')) && IS_MAINTAINER
    if(!has3FilesInArt && (!IS_MAINTAINER || isMaintainerAsContributor)){
        feedbackPush(`Missing files in your contribution: include all required files inside the \`Art/\` folder - (3 files expected)`)
    }
    ...
    for (const file of incorrectFiles){
        ...
        if ( isPictural && !isIcon ) reason = 'pictures are not allowed, use online ones'
        ...
        const isFeedbackForUser = (isContributorInsideArt || isContributorOutsideArt) || isMaintenerInsideArt
...
const checkContent = async (contributionStates) => {
    ...
    const changedFilesWithSpace = changedFiles.filter(filename => /\s/.test(filename))
    if(changedFilesWithSpace.length){
        feedbackPush('Space(s) detected in file or folder name(s) - please remove them')
    }


    const changedFilesWithoutSpace = changedFiles.filter(filename => !/\s/.test(filename))
    ...
    for( const file of changedFilesWithoutSpace ){
        ...
        if(!fileContent && !shouldSkip ){
            ...
            HTMLReviews   = [ ...HTMLReviews,   ...(reviewFile(file, fileContent, checkHTML) || []) ]
            CSSReviews    = [ ...CSSReviews,    ...(reviewFile(file, fileContent, checkCSS)  || []) ]
            JSONReviews   = [ ...JSONReviews,   ...(reviewFile(file, fileContent, checkJSON) || []) ] 
        }
    }
...
const generateReviewMessage = ( feedbackList ) => {
    ...
    const messageLines = [`Aloha ${ interlocutor } 🙌 - Thanks for your contribution!`]
    const messagePush = createFeedbackList(messageLines, 'line')

    if( feedbackList.length ) {
        messagePush("Before we can merge your submission, please address the following points.")
        messagePush("\n## Feedback")

        // Tip message refering to README
        const readmeLink = '[README.md](https://github.com/zero-to-mastery/Animation-Nation/blob/master/README.md)'
